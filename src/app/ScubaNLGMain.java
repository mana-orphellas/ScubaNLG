...
/**
 * This is the main class for the ScubaNLG application. This class has functionality for reading and displaying dive log and profile information from the database.
 * @author csc242
 */
public class ScubaNLGMain extends javax.swing.JFrame {
    ...
    MyJDBC dataBase;
    Vector<Double> x = new Vector<Double>(),y=new Vector<Double>();
    Double diveNo;
    ...
    public ScubaNLGMain() {
        ...
        dataBase = new MyJDBC("jdbc:mysql://localhost:8889/scuba","com.mysql.jdbc.Driver","root","root");
        
        dataBase.executeQuery("Select * from alladinlogs");

        for(int i = 0;i<dataBase.getRowCount();i++){
         x.add(new Double(Utils.convertLongitude((String)dataBase.getValueAt(i,9))));
    ...
    /**
     * Reads the profile information for the dive selected by the user in the map view 
     */
    public TreeMap<Long,Double> readDiveProfile(int row){
        ...
        dataBase.executeQuery("Select Time, Depth from alladin"+diveNo.intValue());
        for(int i = 0;i<dataBase.getRowCount();i++){
        	profile.put(Utils.getTimeInSeconds((String)dataBase.getValueAt(i, 0)),(Double)dataBase.getValueAt(i, 1));
        }
...
public void displayReport(TreeMap<Long,Double> diveProfile, String text){
    ...
    LinePlot gPanel = new LinePlot(0,0,last+120,maxDepth.doubleValue()+10,diveProfile,first,maxDepth);

    javax.swing.JDialog dialog = new javax.swing.JDialog(this,"Dive Report "+diveNo.intValue());
    dialog.add(gPanel,java.awt.BorderLayout.CENTER);
    javax.swing.JEditorPane textPane = new javax.swing.JEditorPane("text/html","");
    
    javax.swing.JScrollPane scrollPane = new javax.swing.JScrollPane(textPane); 
